import { dashboardIndicators } from "../../../presentation/layout/pages/area-logista/dashboard/data/dashboardCharts";
...
export function useDashboardData() {
  const indicators: IndicatorData[] = dashboardIndicators.map(
    (current, index) => {
  ...
  const getChartData = (indicatorName: string) => {
    return (
      chartDataByIndicator[
        indicatorName as keyof typeof chartDataByIndicator
      ] || []
    );
  };
